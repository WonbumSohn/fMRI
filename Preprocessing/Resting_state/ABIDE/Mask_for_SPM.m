...
%   To modify so that the threshold can be set individually for each mask area
%   Ver. May-2nd-2023
...
for subji=1:length(subj_list)
    ...
    for sess_iter = 1:length(sess_lists)
        ...
        for seg_iter = 1:length(seg_types)
            ...
            now_threshold = threshold_value{seg_iter} ;
            ...
            matlabbatch{1}.spm.util.imcalc.expression = ['i2>' num2str(now_threshold)] ;
            ...
            %   
            cprintf('black', 'The %s mask is created with threshold %d.\n', now_mask_type, now_threshold)
        ...
        %   Remove the variables repeating or not necessary
        clear func_folder_path func_data     
    end
