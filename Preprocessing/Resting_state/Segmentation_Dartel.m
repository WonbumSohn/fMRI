...
%   By Wonbum Sohn
%   Ver. April-30th-2023
%   For segmentation of anatomical data with the dartel results.
...
%   
cprintf('red', '<<<<<<<<<< Started Segmentation with Dartel >>>>>>>>>>\n') ;
...
for subji=1:length(subj_list)
    ...
    for sess_iter = 1:length(sess_lists)
        ...
        matlabbatch{1}.spm.spatial.preproc.tissue(1).tpm = {fullfile(now_grp_path, 'GM_6.nii')} ;
        matlabbatch{1}.spm.spatial.preproc.tissue(1).ngaus = 1;
        matlabbatch{1}.spm.spatial.preproc.tissue(1).native = [1 1];
        matlabbatch{1}.spm.spatial.preproc.tissue(1).warped = [1 1];
        matlabbatch{1}.spm.spatial.preproc.tissue(2).tpm = {fullfile(now_grp_path, 'WM_6.nii')} ;
        matlabbatch{1}.spm.spatial.preproc.tissue(2).ngaus = 1;
        matlabbatch{1}.spm.spatial.preproc.tissue(2).native = [1 1];
        matlabbatch{1}.spm.spatial.preproc.tissue(2).warped = [1 1];
        matlabbatch{1}.spm.spatial.preproc.tissue(3).tpm = {fullfile(now_grp_path, 'CSF_6.nii')} ;
...
%
cprintf('red', '<<<<<<<<<< Finished Segmentation with Dartel >>>>>>>>>>\n') ;

%   For calculation of duration time
tEnd = toc(tStart) ;
%   Save the duration time of this step.
file_ID = fopen(fullfile(data_basic_path, 'processing_record.txt'), 'a') ;              %   Set up txt file location and name
content_format = 'The duration time of segmentation with dartel is %d seconds.\n' ;     %   Set up a form to fill in the txt file
fprintf(file_ID, content_format, tEnd) ;                                                %   Enter values in txt file
fclose(file_ID) ;
%
cprintf('black', 'The duration time of segmentation with dartel is %d seconds.\n\n\n', tEnd) ;
